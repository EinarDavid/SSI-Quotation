...
export async function getWeekOfYear(year, month) {
    return await axios.get(api + '/ssiCotizacionGetWeeksOfyear/' + year + '/' + month);
}

export async function getWeekOfYearOfWeek(year, month, week) {
    return await axios.get(api + '/ssiCotizacionGetLabourDaysInformation/' + year + '/' + month + '/'+ week);
}

export async function getHoursResource(resource, year, month, week ) {
    return await axios.get(api + '/ssiCotizacionGetResourceAllocationDetail/'+ resource + '/'  + year + '/' + month + '/'+ week);
}
